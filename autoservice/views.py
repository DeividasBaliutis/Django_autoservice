    ...
    if request.method == "POST":
        ...
        if password == password2:
            if User.objects.filter(username=username).exists():
                ...
                messages.info(request, _('Username %s registered!') % username)
...
class KlientoUzsakymas(LoginRequiredMixin, generic.DetailView):
    model = Order
    template_name = 'vartotojo_uzsakymas.html'

class OrderByUserCreateView(LoginRequiredMixin, generic.CreateView):
    model = Order
    fields = ['car', 'return_date']
    success_url = "/autoservice/mano_uzsakymai/"
